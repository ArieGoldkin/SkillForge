---
description: "Complexity scoring, blocking thresholds, gate validation, and stuck detection"
alwaysApply: false
---
...
Complexity scoring, blocking thresholds, gate validation, and stuck detection

## When to apply

- Apply when the task matches the capability keywords/triggers for this skill.
- If unsure, open `@.claude/skills/quality-gates/capabilities.json` and check `capabilities.*.solves`.

Trigger hints from `capabilities.json` (keep this lightweight): `quality.*gate, block.*task, complexity.*score, stuck, escalate`
...
- `@.claude/skills/quality-gates/capabilities.json` (discovery + triggers)
- `@.claude/skills/quality-gates/SKILL.md` (overview + patterns)
- Templates: `@.claude/skills/quality-gates/templates/`
