...
SSE, WebSockets, ReadableStream for real-time data, LLM streaming, and backpressure handling

## When to apply

- Apply when the task matches the capability keywords/triggers for this skill.
- If unsure, open `@.claude/skills/streaming-api-patterns/capabilities.json` and check `capabilities.*.solves`.

Trigger hints from `capabilities.json` (keep this lightweight): `stream.*response, real-time.*data, sse.*implement, websocket.*setup, live.*update, push.*notification`
...
- `@.claude/skills/streaming-api-patterns/capabilities.json` (discovery + triggers)
- `@.claude/skills/streaming-api-patterns/SKILL.md` (overview + patterns)
- Templates: `@.claude/skills/streaming-api-patterns/templates/`
- Checklists: `@.claude/skills/streaming-api-patterns/checklists/`
