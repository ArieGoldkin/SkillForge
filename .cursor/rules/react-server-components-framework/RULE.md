---
description: "Next.js 15 App Router, RSC patterns, Server Actions, streaming SSR, and modern data fetching"
alwaysApply: false
globs:
  - frontend/**/*.{ts,tsx}
---
...
Next.js 15 App Router, RSC patterns, Server Actions, streaming SSR, and modern data fetching

## When to apply

- Apply when the task matches the capability keywords/triggers for this skill.
- If unsure, open `@.claude/skills/react-server-components-framework/capabilities.json` and check `capabilities.*.solves`.

Trigger hints from `capabilities.json` (keep this lightweight): `next.*js.*15, server.*component, app.*router, server.*action, react.*18, rsc`
...
- `@.claude/skills/react-server-components-framework/capabilities.json` (discovery + triggers)
- `@.claude/skills/react-server-components-framework/SKILL.md` (overview + patterns)
- Browse: `@.claude/skills/react-server-components-framework/references/`
- Templates: `@.claude/skills/react-server-components-framework/templates/`
- Checklists: `@.claude/skills/react-server-components-framework/checklists/`
- Examples: `@.claude/skills/react-server-components-framework/examples/`
