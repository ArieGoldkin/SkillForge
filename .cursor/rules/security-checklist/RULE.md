...
OWASP Top 10 mitigations, authentication patterns, input validation, and compliance guidelines

## When to apply

- Apply when the task matches the capability keywords/triggers for this skill.
- If unsure, open `@.claude/skills/security-checklist/capabilities.json` and check `capabilities.*.solves`.

Trigger hints from `capabilities.json` (keep this lightweight): `security.*audit, owasp, vulnerability, penetration.*test, auth.*security, input.*validation`
...
- `@.claude/skills/security-checklist/capabilities.json` (discovery + triggers)
- `@.claude/skills/security-checklist/SKILL.md` (overview + patterns)
- Checklists: `@.claude/skills/security-checklist/checklists/`
