...
Cloudflare Workers, Vercel Edge, Deno Deploy for globally distributed, low-latency applications

## When to apply

- Apply when the task matches the capability keywords/triggers for this skill.
- If unsure, open `@.claude/skills/edge-computing-patterns/capabilities.json` and check `capabilities.*.solves`.

Trigger hints from `capabilities.json` (keep this lightweight): `edge.*function, cloudflare.*worker, vercel.*edge, edge, cdn`
...
- `@.claude/skills/edge-computing-patterns/capabilities.json` (discovery + triggers)
- `@.claude/skills/edge-computing-patterns/SKILL.md` (overview + patterns)
