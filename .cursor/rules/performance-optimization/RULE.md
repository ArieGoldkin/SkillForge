...
- If unsure, open `@.claude/skills/performance-optimization/capabilities.json` and check `capabilities.*.solves`.

Trigger hints from `capabilities.json` (keep this lightweight): `optimize.*performance, slow.*app, bundle.*size, query.*slow, faster, performance`
...
- `@.claude/skills/performance-optimization/capabilities.json` (discovery + triggers)
- `@.claude/skills/performance-optimization/SKILL.md` (overview + patterns)
- Templates: `@.claude/skills/performance-optimization/templates/`
