---
description: "Structured logging, metrics collection, distributed tracing, and alerting strategies"
alwaysApply: false
---
...
Structured logging, metrics collection, distributed tracing, and alerting strategies

## When to apply

- Apply when the task matches the capability keywords/triggers for this skill.
- If unsure, open `@.claude/skills/observability-monitoring/capabilities.json` and check `capabilities.*.solves`.

Trigger hints from `capabilities.json` (keep this lightweight): `monitoring.*setup, logging.*structured, tracing.*distributed, observe, metrics`
...
- `@.claude/skills/observability-monitoring/capabilities.json` (discovery + triggers)
- `@.claude/skills/observability-monitoring/SKILL.md` (overview + patterns)
- Templates: `@.claude/skills/observability-monitoring/templates/`
