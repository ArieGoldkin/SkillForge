...
Comprehensive API design patterns, versioning strategies, error handling conventions, and OpenAPI templates

## When to apply

- Apply when the task matches the capability keywords/triggers for this skill.
- If unsure, open `@.claude/skills/api-design-framework/capabilities.json` and check `capabilities.*.solves`.

Trigger hints from `capabilities.json` (keep this lightweight): `design.*api, create.*endpoint, add.*route, api.*pattern, backend.*api, http.*method`
...
- `@.claude/skills/api-design-framework/capabilities.json` (discovery + triggers)
- `@.claude/skills/api-design-framework/SKILL.md` (overview + patterns)
- Templates: `@.claude/skills/api-design-framework/templates/`
- Checklists: `@.claude/skills/api-design-framework/checklists/`
