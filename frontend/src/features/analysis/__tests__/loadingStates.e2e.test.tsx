...
// Render helper with proper provider setup for E2E testing
...
if (environmentCapabilities.e2eReady) {
  describe('Loading States E2E Workflows', () => {
    ...
    describe('Happy Path: Medium Analysis (10-30 seconds)', () => {
      it('shows timeout warning during longer analysis', async () => {
        ...
        await waitFor(() => {
          expect(
            screen.getByText(/This analysis is taking longer than expected/)
          ).toBeInTheDocument()
        })
    ...
    describe('Error Path: Analysis Failure', () => {
      it('handles analysis processing failures', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByText('Analysis failed')).toBeInTheDocument()
          expect(
            screen.getByText('AI processing failed: Invalid content format')
          ).toBeInTheDocument()
        })
      })
    })
  ...
  console.log('E2E tests skipped: Set E2E_READY=true to enable E2E workflow tests')
}
