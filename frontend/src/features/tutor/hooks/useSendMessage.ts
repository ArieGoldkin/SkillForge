...
export function useSendMessage({ sessionId, messages, setMessages }: UseSendMessageProps) {
  return useMutation({
    mutationFn: (content: string) => mockTutoringAPI.sendMessage(sessionId, content),
