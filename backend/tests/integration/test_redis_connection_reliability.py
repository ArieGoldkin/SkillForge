...
def test_connection_pool_health():
    ...
    try:
        ...
        logger.info("test_complete", test="connection_pool_health", status="PASS", pings=10)
    finally:
        client.close()
...
def test_connection_configuration():
    ...
    try:
        ...
        assert pool.connection_kwargs.get("socket_timeout") == 5, "Socket timeout should be 5s"

        assert pool.connection_kwargs.get("health_check_interval") == 30, (
            "Health check interval should be 30s"
        )

        assert pool.max_connections == 20, "Max connections should be 20"
    ...
    try:
        ...
        if pm.redis_client:
            with patch.object(pm.redis_client, "get", side_effect=mock_get_with_errors):
                ...
                result = await pm._get_from_l2_cache(name="test-prompt", label="production")
