...
class TestSemanticExampleSelectorIntegration:
    ...
    async def test_examples_by_agent_type(self, db_session, minimal_agent_examples):
        ...
        for agent_type in expected_types:
            assert agent_type in counts_by_type, f"Missing examples for agent_type: {agent_type}"
            assert counts_by_type[agent_type] > 0, f"No examples for agent_type: {agent_type}"
    ...
    async def test_embeddings_are_populated(self, db_session):
        ...
        result = await db_session.execute(
            text("SELECT COUNT(*) FROM agent_examples WHERE embedding IS NULL OR embedding = '[]'")
        )
    ...
    async def test_semantic_selector_retrieves_examples(self, db_session, requires_llm):
        ...
        content = (
            "Comparing React hooks vs class components for state management in modern applications"
        )
        ...
        assert result.total_candidates >= len(result.examples), (
        ...
        if len(result.examples) > 1:
            ...
            assert distances == sorted(distances), "Examples should be ordered by similarity"

        # Print results for visibility
        print(f"\nRetrieved {len(result.examples)} examples:")
        for i, example in enumerate(result.examples, 1):
            print(
                f"  {i}. Quality: {example.quality_score:.2f}, "
    ...
    async def test_selector_with_different_agent_types(self, db_session, requires_llm):
        ...
        for agent_type, content in test_cases:
            ...
            assert result.total_candidates > 0, f"No candidates for {agent_type}"
    ...
    async def test_selector_performance_benchmarking(self, db_session, requires_llm):
        ...
        assert avg_latency < 10000, f"Average latency too high: {avg_latency:.2f}ms"
    ...
    async def test_selector_with_quality_threshold(self, db_session, requires_llm):
        ...
        for threshold in thresholds:
            ...
            for example in result.examples:
                assert example.quality_score >= threshold, (
...
class TestFewShotAgentFactoryIntegration:
    ...
    async def test_performance_end_to_end(self, db_session, requires_llm):
        ...
        # More realistic for CI environments with potential network latency
        assert total_time_ms < 10000, f"Creation too slow: {total_time_ms:.2f}ms"
...
class TestFewShotRealWorldScenarios:
    ...
    async def test_similarity_relevance_scores(self, db_session, requires_llm):
        ...
        # Very specific query
        content = (
            "Implementing OAuth 2.0 authentication flow with PKCE for single-page applications"
        )
