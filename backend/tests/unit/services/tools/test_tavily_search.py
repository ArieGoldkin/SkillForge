...
async def test_search_basic_depth(tavily_search):
    ...
    result = await tavily_search.search_basic("test query")

    assert result["query"] == "test query"
    # Verify request was made with basic depth
    call_args = tavily_search.client.post.call_args
    assert call_args[1]["json"]["search_depth"] == "basic"
...
async def test_search_advanced_depth(tavily_search):
    ...
    result = await tavily_search.search_advanced("test query")

    assert result["query"] == "test query"
    # Verify request was made with advanced depth
    call_args = tavily_search.client.post.call_args
...
async def test_search_custom_max_results(tavily_search):
    ...
    await tavily_search.search("test query", max_results=10)

    # Verify request included max_results parameter
    call_args = tavily_search.client.post.call_args
...
async def test_search_include_raw_content(tavily_search):
    ...
    await tavily_search.search("test query", include_raw_content=True)

    # Verify request included include_raw_content parameter
    call_args = tavily_search.client.post.call_args
    assert call_args[1]["json"]["include_raw_content"] is True
...
async def test_search_include_images(tavily_search):
    ...
    await tavily_search.search("test query", include_images=True)

    # Verify request included include_images parameter
    call_args = tavily_search.client.post.call_args
    assert call_args[1]["json"]["include_images"] is True
...
async def test_search_no_answer(tavily_search):
    ...
    result = await tavily_search.search("test query", include_answer=False)

    # Should not include answer in request
    call_args = tavily_search.client.post.call_args
    assert call_args[1]["json"]["include_answer"] is False
    # Result should handle missing answer gracefully
