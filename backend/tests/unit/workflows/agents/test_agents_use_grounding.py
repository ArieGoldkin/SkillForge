...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("integration_feasibility", "run_integration_feasibility", "INTEGRATION_FEASIBILITY_PROMPT"),
    ],
)
async def test_agent_applies_grounding(
    ...
    with (
        patch(f"app.workflows.agents.{agent_module}.create_structured_agent", side_effect=mock_create_agent),
        patch(f"app.workflows.agents.{agent_module}.run_agent_with_tracking", new_callable=AsyncMock, return_value=mock_tracking_result),
    ):
        # Run the agent
        await agent_func(
...
@pytest.mark.asyncio
async def test_tech_comparator_uses_grounding(mock_session: AsyncSession, base_state: AnalysisState):
    ...
    with (
        ...
        patch("app.workflows.agents.tech_comparator.run_agent_with_tracking", new_callable=AsyncMock),
    ):
...
@pytest.mark.asyncio
async def test_security_auditor_uses_grounding(mock_session: AsyncSession, base_state: AnalysisState):
    ...
    with (
        ...
        patch("app.workflows.agents.security_auditor.run_agent_with_tracking", new_callable=AsyncMock),
    ):
        mock_apply.return_value = "grounded_prompt"
...
@pytest.mark.asyncio
async def test_implementation_planner_uses_grounding(mock_session: AsyncSession, base_state: AnalysisState):
    ...
    with (
        ...
        patch("app.workflows.agents.implementation_planner.run_agent_with_tracking", new_callable=AsyncMock),
    ):
        mock_apply.return_value = "grounded_prompt"
    ...
    for module_name in agent_modules:
        ...
        if source:
            with open(source, 'r') as f:
                ...
                assert "apply_grounding(" in content, (
                    f"{module_name} does not call apply_grounding"
                )
