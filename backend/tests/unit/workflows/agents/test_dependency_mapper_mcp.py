"""Unit tests for Dependency Mapper MCP tool integration (Issue #234).

This test suite validates MCP tool integration for the dependency_mapper agent:
- Conditional agent creation (tool-enabled vs structured-only)
- Tool loading in run_dependency_mapper_with_session
...
@pytest.fixture
def mock_tools():
    """Create mock MCP tools for dependency mapper."""
    tool1 = MagicMock(spec=BaseTool)
    tool1.name = "npm_get_package"
    tool1.description = "Get npm package metadata"

    tool2 = MagicMock(spec=BaseTool)
    tool2.name = "pypi_get_package"
    tool2.description = "Get PyPI package info"

    tool3 = MagicMock(spec=BaseTool)
    tool3.name = "github_get_repo"
    tool3.description = "Get GitHub repository details"
...
class TestRunDependencyMapperWithTools:
    ...
    async def test_runs_agent_with_tracking(
        ...
        assert call_kwargs["agent_type"] == "dependency_mapper"
        assert call_kwargs["session"] == mock_session

        # Verify result is returned
        assert result == {"agent_type": "dependency_mapper", "findings": {}}


# ============================================================================
# TestDependencyMapperWithSessionMCP - Session Runner Integration
# ============================================================================
...
class TestDependencyMapperWithSessionMCP:
    ...
    def test_runner_checks_registry_enabled(self):
        ...
        assert '"dependency_mapper"' in source
