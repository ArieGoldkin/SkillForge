...
class TestSpecificityScorer:
    ...
    def test_vague_output_low_score(self) -> None:
        ...
        assert result.vague_phrase_count >= 4, (
            f"Expected many vague phrases, got {result.vague_phrase_count}"
        )
        ...
        assert result.numeric_value_count == 0, (
            f"Expected 0 numeric values (no units), got {result.numeric_value_count}"
        )
    ...
    def test_specific_output_high_score(self) -> None:
        ...
        assert result.vague_phrase_count <= 2, (
            f"Expected few vague phrases, got {result.vague_phrase_count}"
        )
        assert result.numeric_value_count >= 15, (
            f"Expected many numeric values, got {result.numeric_value_count}"
        )
    ...
    def test_numeric_field_compliance_performance_analyst(self) -> None:
        ...
        output = {
            "performance_metrics": [
                ...
                {
                    "metric_name": "throughput",
                    "current_value": "5000 req/sec",
                    "target_value": ">10000 req/sec",
                    "notes": "Notes",
                },
            ],
        ...
        assert result.numeric_field_compliance >= 0.9, (
            f"Expected high compliance, got {result.numeric_field_compliance}"
        )
    ...
    def test_numeric_field_compliance_missing_units(self) -> None:
        ...
        assert result.numeric_field_compliance < 0.5, (
            f"Expected low compliance, got {result.numeric_field_compliance}"
        )
    ...
    def test_vague_phrase_detection(self) -> None:
        ...
        assert result.vague_phrase_count >= 8, (
            f"Expected many vague phrases, got {result.vague_phrase_count}"
        )
    ...
    def test_numeric_value_detection(self) -> None:
        ...
        assert result.numeric_value_count >= 10, (
            f"Expected many numeric values, got {result.numeric_value_count}"
        )
    ...
    def test_implementation_planner_specific_fields(self) -> None:
        ...
        assert result.numeric_value_count >= 4, (
            f"Expected time values, got {result.numeric_value_count}"
        )
    ...
    def test_security_auditor_cvss_scores(self) -> None:
        ...
        assert result.overall_score >= 0.50, f"Expected moderate+ score, got {result.overall_score}"
        assert result.numeric_value_count >= 4, (
            f"Expected CVSS scores and times, got {result.numeric_value_count}"
        )
    ...
    def test_quality_level_classification(self) -> None:
        ...
        excellent_output = {
            "metric": "450ms",
            "target": "<200ms",
            "improvement": "reduce by 250ms (55%)",
        }
    ...
    def test_get_detailed_report(self) -> None:
        ...
        output = {"description": "Use appropriate caching. Latency: 450ms. Target: <200ms."}
    ...
    def test_nested_structure_flattening(self) -> None:
        ...
        assert result.numeric_value_count >= 2, (
            f"Expected numeric values in nested structure, got {result.numeric_value_count}"
        )
        assert result.vague_phrase_count >= 1, (
            f"Expected vague phrase in nested structure, got {result.vague_phrase_count}"
        )
    ...
    def test_range_and_estimate_patterns(self) -> None:
        ...
        assert result.numeric_value_count >= 5, (
            f"Expected ranges and estimates, got {result.numeric_value_count}"
        )
    ...
    def test_comparison_operators(self) -> None:
        ...
        assert result.numeric_value_count >= 4, (
            f"Expected comparison values, got {result.numeric_value_count}"
        )

    def test_version_numbers(self) -> None:
        """Test detection of version numbers."""
        output = {"versions": "Node.js: 18.0.0, React: 18.2.0, PostgreSQL: 14.5.0"}

        scorer = SpecificityScorer()
        result = scorer.score_output(output)

        # Should detect version numbers
        assert result.numeric_value_count >= 3, (
            f"Expected version numbers, got {result.numeric_value_count}"
        )
    ...
    def test_cve_references(self) -> None:
        ...
        assert result.numeric_value_count >= 4, (
            f"Expected CVEs and CVSS scores, got {result.numeric_value_count}"
        )
    ...
    def test_vague_with_numbers_still_specific(self) -> None:
        ...
        assert result.numeric_value_count >= 3, (
            f"Expected numeric values, got {result.numeric_value_count}"
        )
        ...
        assert result.overall_score >= 0.5, (
            f"Expected reasonable score due to numeric values, got {result.overall_score}"
        )
    ...
    def test_expected_numeric_count_varies_by_agent(self) -> None:
        ...
        assert result_perf.expected_numeric_count >= 9, (
            f"Performance analyst should expect many values, got {result_perf.expected_numeric_count}"
        )

        # Unknown agent has default expectation
        result_unknown = scorer.score_output(output, agent_type="unknown")
        assert result_unknown.expected_numeric_count == 10, (
            f"Unknown agent should have default expectation, got {result_unknown.expected_numeric_count}"
        )
...
class TestSpecificityIntegration:
    ...
    def test_real_performance_analyst_output(self) -> None:
        ...
        assert score.numeric_value_count >= 15, (
            f"Expected many numeric values, got {score.numeric_value_count}"
        )
        assert score.vague_phrase_count <= 3, (
            f"Expected few vague phrases, got {score.vague_phrase_count}"
        )
    ...
    def test_real_vague_performance_analyst_output(self) -> None:
        ...
        assert score.vague_phrase_count >= 4, (
            f"Expected many vague phrases, got {score.vague_phrase_count}"
        )
