...
async def test_extraction_handles_missing_gracefully():
    ...
    with (
        patch(
            "app.domains.analysis.workflows.agents.base.save_agent_finding", new_callable=AsyncMock
        ),
        patch(
...
async def test_extraction_handles_invalid_type_gracefully():
    ...
    with (
        ...
        patch(
            "app.shared.services.messaging.sse_helpers.persist_progress_event_async",
            new_callable=AsyncMock,
        ),
    ):
