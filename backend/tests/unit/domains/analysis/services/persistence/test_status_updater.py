...
async def test_status_updater_update_success(mock_analysis_id):
    ...
    # Verify status was updated to "extracting" (not "complete")
    assert mock_analysis.status == "extracting"
...
async def test_status_updater_update_database_error(mock_analysis_id):
    """Test status update re-raises database errors (intentional behavior)."""
    mock_db_session = AsyncMock()
    ...
    with patch(
        ...
        updater = StatusUpdater()
        # Should raise - database errors are re-raised (intentional behavior)
