...
async def test_generate_embedding_token_counting_accuracy(
    ...
    # Use a single token word to avoid encoding variations
    single_token = "test"  # Verify this is one token
    assert len(encoding.encode(single_token)) == 1, "Test setup: single_token should be 1 token"
    test_text = (single_token + " ") * 8_000
    tokens = encoding.encode(test_text)
    # Allow off-by-one due to encoding rounding
    assert len(tokens) in (8_000, 8_001), (
        f"Test setup: text should have 8,000-8,001 tokens (got {len(tokens)})"
    )
    ...
    with patch.object(
        ...
        # Verify no truncation occurred (at or below limit)
        call_args = mock_create.call_args
        sent_text = call_args.kwargs["input"]
        sent_tokens = encoding.encode(sent_text)
        assert len(sent_tokens) <= MAX_TOKENS, (
            f"Text should not exceed {MAX_TOKENS} tokens (got {len(sent_tokens)})"
        )
        assert len(result) == EXPECTED_EMBEDDING_DIMENSIONS
...
async def test_embedding_service_requires_api_key() -> None:
    ...
    with patch("app.services.embeddings.service.settings") as mock_settings:
