...
def get_langfuse_client() -> Any:
    ...
    Returns:
        Langfuse SDK client instance or None
...
def get_langfuse_callback_handler() -> Any:
    """Get Langfuse CallbackHandler (backward compatibility).

    DEPRECATED: Use get_langfuse_service().get_callback_handler() instead.
    ...
    """
    service = get_langfuse_service()
    return service.get_callback_handler() if service else None
...
def submit_langfuse_score(
    *,
    ...
    """Submit a score to Langfuse (backward compatibility).

    DEPRECATED: Use get_langfuse_service().submit_score() instead.
    ...
    service = get_langfuse_service()
    if service:
        service.submit_score(trace_id=trace_id, name=name, value=value, comment=comment)


def flush_langfuse() -> None:
    """Flush pending Langfuse events (backward compatibility).

    DEPRECATED: Use get_langfuse_service().flush() instead.
...
async def close_langfuse_client() -> None:
    """Close Langfuse client (backward compatibility).

    DEPRECATED: Use shutdown_langfuse_service() instead.

    """
    await shutdown_langfuse_service()
