...
@observe(as_type="agent", name="performance_analyst")
async def performance_analyst_node(state: AnalysisState) -> dict[str, object]:
    ...
    try:
        ...
        await emit_agent_progress(
            ...
            error_code="PERFORMANCE_ANALYST_TIMEOUT",
            processing_time_ms=processing_time_ms,
        )
        logger.warning(
            "performance_analyst_timeout",
            analysis_id=str(analysis_id),
            duration_seconds=duration,
        ...
        logger.warning(
            "performance_analyst_specificity_failed",
            analysis_id=str(analysis_id),
            error=str(e),
            duration_seconds=duration,
        )
        ...
        await emit_agent_progress(
            ...
            "failed",
            error=str(e),
            error_code="PERFORMANCE_ANALYST_FAILED",
            processing_time_ms=processing_time_ms,
        )
