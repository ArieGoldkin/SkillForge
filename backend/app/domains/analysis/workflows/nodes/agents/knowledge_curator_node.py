"""Knowledge curator agent node for LangGraph StateGraph.
...
async def knowledge_curator_node(state: AnalysisState) -> dict[str, object]:
    ...
    Issue #500: Tier 3 Research agent that uses proactive memory injection
    (prior_memory field) to connect content to user's existing knowledge graph.
    ...
    Args:
        state: Current workflow state with content_ref/raw_content and prior_memory
    ...
    try:
        ...
        # Issue #500: Import runner function for knowledge curator
        from app.domains.analysis.workflows.tasks.runners import (
            run_knowledge_curator_with_session,
        )

        result = await run_knowledge_curator_with_session(
        ...
        await emit_agent_progress(
            ...
            "failed",
            error=str(e),
            error_code="KNOWLEDGE_CURATOR_FAILED",
            processing_time_ms=processing_time_ms,
        )
