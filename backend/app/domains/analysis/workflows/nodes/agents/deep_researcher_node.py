"""Deep researcher agent node for LangGraph StateGraph.
...
Issue #500: Tier 3 Research agent with memory + tools (most powerful combination).
...
async def deep_researcher_node(state: AnalysisState) -> dict[str, object]:  # noqa: PLR0915 - Agent node requires comprehensive error handling
    ...
    research depth. This is the most powerful research capability.
    ...
    Args:
        state: Current workflow state with content_ref/raw_content, prior_memory, and tools
    ...
    update_current_trace(
        ...
        tags=["parallel-execution", "tier-3-research", "memory-enabled", "tool-enabled"],
        session_id=f"analysis-{analysis_id}",
        user_id="anonymous",
    )
    trace_id = get_current_trace_id()

    # Log memory and tool availability
    ...
    try:
        ...
        # Issue #500: Import runner function for deep researcher
        from app.domains.analysis.workflows.tasks.runners import (
        ...
        await emit_agent_progress(
            ...
            "failed",
            error=str(e),
            error_code="DEEP_RESEARCHER_FAILED",
            processing_time_ms=processing_time_ms,
        )
