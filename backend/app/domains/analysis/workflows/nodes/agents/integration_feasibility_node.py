...
@observe(as_type="agent", name="integration_feasibility", capture_input=True, capture_output=True)
async def integration_feasibility_node(state: AnalysisState) -> dict[str, object]:
    ...
    try:
        ...
        agent_meta = get_agent_metadata("integration_feasibility")
        tier = agent_meta.tier if agent_meta else 2  # Tier 2 VALIDATION

        result = await execute_with_resilience(
            agent_type="integration_feasibility",
            tier=tier,
            fn=lambda: run_integration_feasibility_with_session(
