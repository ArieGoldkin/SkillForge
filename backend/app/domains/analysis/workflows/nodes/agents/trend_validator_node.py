...
@observe(as_type="agent", name="trend_validator", capture_input=True, capture_output=True)
async def trend_validator_node(state: AnalysisState) -> dict[str, object]:
    """Execute trend validation analysis.
    ...
    try:
        ...
        await emit_agent_progress(
            ...
            error_code="TREND_VALIDATOR_TIMEOUT",
            processing_time_ms=processing_time_ms,
        )
        logger.warning(
            "trend_validator_timeout",
            analysis_id=str(analysis_id),
            duration_seconds=duration,
        ...
        logger.warning(
            "trend_validator_specificity_failed",
            analysis_id=str(analysis_id),
            error=str(e),
            duration_seconds=duration,
        )
        ...
        await emit_agent_progress(
            ...
            "failed",
            error=str(e),
            error_code="TREND_VALIDATOR_FAILED",
            processing_time_ms=processing_time_ms,
        )
